...
class followerEnv(baseFourRoomsEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        # (fixed_coord, is_vertical, lo, hi) — lo/hi are the exclusive-end
        # ranges of positions along the wall segment where a door can sit.
        door_slots = [
            (room_w, True,  1,          room_h),        # vertical, top half
            (room_w, True,  room_h + 1, height - 1),    # vertical, bottom half
            (room_h, False, 1,          room_w),        # horizontal, left half
            (room_h, False, room_w + 1, width - 1),     # horizontal, right half
        ]

        for fixed, is_vert, lo, hi in door_slots:
            if self.np_random.random() >= self.door_shift_prob:
                continue
            # Close the existing door gap in this segment
            for pos in range(lo, hi):
                x, y = (fixed, pos) if is_vert else (pos, fixed)
                if self.grid.get(x, y) is None:
                    self.grid.set(x, y, Wall())
                    break
            # Open a new random position on the same segment
            new_pos = int(self.np_random.integers(lo, hi))
            x, y = (fixed, new_pos) if is_vert else (new_pos, fixed)
